...
# 🔧 ОБНОВЛЕНИЕ ЛОГИКИ ПРОВЕРКИ ПЛАТЕЖЕЙ
# Исправление проблемы с проверкой статуса платежа

echo "🔧 ОБНОВЛЕНИЕ ЛОГИКИ ПРОВЕРКИ ПЛАТЕЖЕЙ"
echo "=" * 50
...
# 4. Тестирование исправленной логики
echo "🧪 Тестирование исправленной логики..."
python3 test_payment_flow_fixed.py
...
if [ $? -eq 0 ]; then
    ...
    echo ""
    echo "🔧 ИСПРАВЛЕНИЯ:"
    echo "   ✅ Проверка платежа теперь работает через базу данных"
    echo "   ✅ Webhook правильно обрабатывает данные от Prodamus"
...
echo ""
echo "🎉 ЛОГИКА ПРОВЕРКИ ПЛАТЕЖЕЙ ОБНОВЛЕНА!"
echo "✅ Теперь бот будет правильно проверять статус платежа"
