...
class ProdаmusAPI:
    ...
    def create_payment(self, order_id: str, amount: int, description: str, user_id: int) -> Optional[str]:
        ...
        try:
            ...
            # Создаем подпись используя официальную библиотеку
    ...
    def verify_webhook(self, data: Dict, signature: str) -> bool:
        """Проверка подписи webhook от Продамус используя официальную библиотеку"""
        try:
            print(f"Проверка подписи:")
            print(f"  Полученная подпись: {signature}")
            print(f"  Данные: {data}")
            
            # Используем официальную библиотеку Hmac для проверки
            is_valid = Hmac.verify(data, self.secret_key, signature)
            
            if is_valid:
                print(f"  ✅ Подпись корректна (проверена через официальную библиотеку Hmac)")
                return True
            else:
